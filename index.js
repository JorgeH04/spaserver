...
const taskRouter = require('./api/recursos/task/task');
const taskdosRouter = require('./api/recursos/taskdos/taskdos');
...
app.use('/api/post', [taskRouter, likesRouter]);
app.use(taskdosRouter);
...
if (process.env.NODE_ENV === 'development') {
  server = app.listen(config.puerto, () => {
    logger.info(`Listen on port ${config.puerto}.`);
  });
} else {
  app.listen();
}
